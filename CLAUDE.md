...
- 🔲 **Pendiente: quitar "Estructura de equipos del área (admin)" del panel** (decidido por el usuario). Removición grande (orfana mucho estado/handlers: showEquipAdmin, useEquipmentForArea, EquipmentCard, handleAddEquipmentToArea, etc.) → hacer typecheck-guided en pase aparte. Se PIERDE la gestión admin de equipos en el hub hasta moverla al sidebar.
