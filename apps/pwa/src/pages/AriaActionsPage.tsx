...
function ActionRow({ action, onViewIncident }: {
  ...
  const typeColors: Record<string, string> = {
    create_incident: 'text-brand-ink bg-blue-500/[0.15] dark:bg-primary/[0.15]',
    update_incident_status: 'text-ink-warn bg-amber-500/[0.15] dark:bg-amber-500/[0.15]',
    search_equipment: 'text-ink-ok bg-emerald-500/[0.15] dark:text-ink-ok dark:bg-green-500/[0.15]',
  ...
  return (
    <div className="flex items-center gap-3 py-3 px-4 border-b border-border last:border-0 hover:bg-muted/30 transition-colors">
      ...
      <div className="flex-shrink-0">
        {action.success ? (
          <CheckCircle className="w-5 h-5 text-ink-ok" />
        ) : (
