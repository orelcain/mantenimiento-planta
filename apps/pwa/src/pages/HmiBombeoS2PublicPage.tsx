 ...
 * - Embebe el HTML standalone /hmi-bombeo-s2-embed.html en iframe
 * - Header con título + fullscreen + QR para compartir
 * - Ruta: /aprendizaje/hmi-bombeo-s2
 *
 * A diferencia de HmiKnuroPublicPage, este HMI NO tiene presets ni tooltips —
 * el ciclo es paramétrico (motor PLC interno del HTML) y se opera con sus
 * propios controles (botones sidebar + Play/Pausa/Reset abajo).
 */

import { useRef, useState, useEffect, useMemo, useCallback } from 'react'
import { BookOpen, QrCode, X, Copy, Check, Maximize, Minimize } from 'lucide-react'
...
export function HmiBombeoS2PublicPage() {
  ...
  const iframeSrc = useMemo(() => {
    ...
    return basePath + 'hmi-bombeo-s2-embed.html?v=' + v
  }, [])
  ...
  return (
    <div ref={containerRef} className="flex flex-col w-screen bg-[#0a1628]" style={{ height: '100dvh' }}>
      ...
      >
        ...
        <span className="text-[#3a5a7a] text-xs hidden sm:inline">— HMI Bombeo Acopio S2 (YAL)</span>
      ...
      {!visualFs && (
        ...
        >
          <p className="text-[9px] text-[#2a4a6a] uppercase tracking-wider">Mantenimiento Industrial — Sistema 2 YAL · Simulador del ciclo PLC</p>
        </div>
      )}
