...
export function NotasAparato({ anclaId, notas, onCrear, onBorrar, onEditar }: Props) {
  ...
  return (
    <div className="flex flex-col gap-3">
      {notas.map((n) => (
        <article key={n.id} className="rounded-card border p-3"
          ...
          {editando?.id === n.id ? (
            <div className="flex flex-col gap-1.5">
              <textarea value={editando.texto} rows={3} maxLength={1500} autoFocus
                        ...
                        className="w-full resize-y rounded-ctl border bg-transparent p-2 text-footnote outline-none"
                        style={{ color: 'var(--lc-ink)', borderColor: 'var(--lc-border)' }} />
              <div className="flex gap-2">
                <button type="button"
                        onClick={() => { void onEditar(n.id, editando.texto).then(() => setEditando(null)) }}
                        className="rounded-ctl px-2.5 py-1 text-footnote font-medium"
                        style={{ background: 'var(--lc-aqua)', color: '#fff' }}>
                  Guardar
                </button>
                <button type="button" onClick={() => setEditando(null)}
                        className="rounded-ctl border px-2.5 py-1 text-footnote"
            ...
            <p className="m-0 whitespace-pre-wrap text-footnote leading-relaxed" style={{ color: 'var(--lc-ink)' }}>
              {n.texto}
            </p>
          )}
              ...
              {n.fotos.map((url) => (
                <a key={url} href={url} target="_blank" rel="noreferrer">
                  <img src={url} alt={`Foto de ${anclaId}`} loading="lazy"
                       className="h-20 w-20 rounded-ctl object-cover" />
                </a>
              ))}
            </div>
          )}
            ...
            {n.creadoPor === uid && (
              <span className="flex gap-0.5">
                <button type="button" onClick={() => setEditando({ id: n.id, texto: n.texto })}
                        title="Editar nota" className="rounded-ctl p-1 hover:opacity-70"
                        style={{ color: 'var(--lc-ink-mid)' }}>
                  <Pencil size={13} />
                </button>
                <button type="button" onClick={() => void onBorrar(n)} title="Borrar nota"
                        className="rounded-ctl p-1 hover:opacity-70" style={{ color: 'var(--lc-danger)' }}>
                  <Trash2 size={13} />
                </button>
              </span>
            )}
      ...
      {uid ? (
        <div className="flex flex-col gap-2 rounded-card border p-3"
          ...
          <input
            ...
            className="w-full rounded-ctl border bg-transparent px-2 py-1.5 font-mono text-footnote outline-none"
            style={{ color: 'var(--lc-ink)', borderColor: 'var(--lc-border)' }}
          />
          ...
          <div className="flex items-center gap-2">
            ...
            <button type="button" onClick={() => void guardar()} disabled={guardando || !texto.trim()}
                    className="flex items-center gap-1.5 rounded-ctl px-3 py-1.5 text-footnote font-medium disabled:opacity-40"
                    style={{ background: 'var(--lc-aqua)', color: '#fff' }}>
          ...
          {error && (
            <p className="m-0 text-footnote" style={{ color: 'var(--lc-danger)' }}>{error}</p>
          )}
        </div>
      ) : (
