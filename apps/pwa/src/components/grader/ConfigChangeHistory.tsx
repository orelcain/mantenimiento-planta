...
function VerdictBadge({ v }: { v: SegmentVerdict }) {
      ...
      <div className="inline-flex items-center gap-1.5 text-caption text-muted-foreground bg-muted rounded-ctl px-2 py-1 mt-1">
        ...
        {v.afterPieces > 0 && (
          <span className="text-muted-foreground">
            ({v.afterPieces} pzas hasta ahora — espera ≥30)
          </span>
        )}
      </div>
    )
  ...
  const visualByStatus = {
    improved: { Icon: TrendingDown, bg: 'bg-emerald-500/[0.15]', border: 'border-transparent' },
    worsened: { Icon: TrendingUp,   bg: 'bg-cat-5-tint/[0.15]',    border: 'border-transparent'    },
    ...
    <li className="flex items-start gap-3 py-2.5 border-b border-border/40 last:border-b-0">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-baseline gap-2 flex-wrap">
          ...
          {undetectedChanges > 0 && (
            <span
              className="inline-flex items-center gap-1 text-caption px-1.5 py-0.5 rounded-full bg-amber-500/[0.15] text-ink-warn border border-transparent font-medium"
