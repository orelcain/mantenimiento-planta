...
export function CurrentShiftChip({ plantLineId, className }: CurrentShiftChipProps) {
  ...
  if (!hasActivity) {
    return (
      <button
        ...
        className={cn(
          'group inline-flex items-center gap-2 px-2.5 py-1.5 rounded-ctl border border-transparent bg-amber-500/[0.15] hover:bg-amber-500/[0.15] transition-colors text-left',
          className,
        )}
  ...
  return (
    <button
      ...
      onClick={() => navigate(href)}
      className={cn(
        'group inline-flex items-center gap-2 px-2.5 py-1.5 rounded-ctl border border-transparent bg-emerald-500/[0.15] hover:bg-emerald-500/[0.15] transition-colors text-left',
        className,
      )}
