...
- ⚠ GOTCHA que habria dejado el aviso mudo: `functions/.env` y `serviceAccountKey.json` estan gitignored — **existen en el clon principal, NO en los worktrees**. Corriendo el vigia desde `D:\a\wt-bitacora` fallaba con ENOENT y el error se leia igual que «todo bien». Ahora `env()` y la credencial se buscan en REPO y, si no estan, en el clon principal.
- ⚠ Al escribir el literal de la ruta POR SCRIPT quedo con UNA barra invertida (`D:\a\APP...`): en JS esa secuencia no es un escape valido y la ruta se convertia en `D:aAPP...`. Se resolvio reusando la constante `CLON_PRINCIPAL`, que ya estaba bien escrita, en vez de duplicar el literal. REGLA: los literales con barras invertidas se escriben con Write/Edit, nunca generados desde otro script (es la tercera vez que el escape muerde en esta sesion).
...
- Lo que se hizo, con lo que Orel eligió:
  1. **Regla de ciclo de vida** en el bucket: `graderUploads/` pasa a COLDLINE a los 90 días (5× más barato, sin borrar nada). ⚠⚠ TRAMPA: la clase de este bucket es **REGIONAL** (nombre legacy de Standard), NO `STANDARD`. La primera regla llevaba `matchesStorageClass: ['STANDARD']` y no habría disparado NUNCA. Quedó `['STANDARD','REGIONAL']`. ⚠ Y no incluir `MULTI_REGIONAL` en esa lista: la API lo interpreta como intento de cambiar la clase del bucket y rechaza con 400.
  2. **Peso de las fotos**: se mantienen los **1600 px** (Orel: «la idea es poder distinguir con calidad, lo más livianas posibles» — la resolución es lo que deja ver el detalle) y baja la CALIDAD de 0.82 a 0.75, con `targetBytes` 350 KB → 250 KB. Medido en el canvas del navegador (el pipeline real) sobre una foto real del turno: 336 KB → ~163 KB a q0.75; comparando los mismos píxeles al 200% se pierde grano, no bordes ni relieve. El valor no es el almacenamiento (centavos) sino la SUBIDA por 4G desde planta.
  3. **Vigía mensual**: `scripts/vigia-almacenamiento.cjs` + `.cmd`, mismo molde que `verificar-arranque-turno` (tarea programada el día 1 a las 09:30, Telegram SOLO si hay algo). Revisa techo (4/8 GB), salto de crecimiento (>1,5 GB entre corridas), fotos huérfanas y que la regla de ciclo de vida siga puesta. ⚠ En el primer intento marcaba «2,14 GB ya en clase fría» porque comparaba `!== 'STANDARD'` y todo el bucket es REGIONAL: la comparación correcta es contra NEARLINE/COLDLINE/ARCHIVE.
- Lo que NO se hizo y por qué: **WebP** pesaría la mitad a igual calidad (123 KB vs 232 KB, medido), pero el correo «copiar normal» referencia la URL del objeto en Storage y Outlook clásico no muestra WebP. El PDF y la lámina de WhatsApp sí convierten por canvas, así que solo el correo lo impide. Queda anotado por si algún día el correo pasa a incrustar siempre.
- Dato sano: CERO fotos huérfanas en `bitacora/` (36 objetos, 36 referenciados): la cola de borrados funciona.
- tsc 0; eslint 30; vitest 2.808 OK; build OK. La tarea programada ya está creada y apunta al repo principal (`D:\a\APP leventamiento...`): empieza a correr cuando ese clon tenga el script (merge + pull).
