...
- Estado: EN REVISIÓN → rama `feat/aria-pwa-escritura-repuestos` (sin commit/PR aún — pendiente decisión de Orel de alcance de rol y prueba en vivo antes de mergear a `main`, que SÍ dispara deploy automático vía GitHub Actions a la app que usan técnicos reales).
- Sigue: Orel prueba logueado en el preview (5181) o local; si OK, PR a main. Follow-up opcional: botones Confirmar/Cancelar dedicados para repuestos (hoy solo texto plano) + soporte de fotos (requiere adaptar la subida a `uploadRepuestoFoto`).
