...
- [ ] **Bitácora de turno de Mantención (`/bitacora`)** — EN PROD, esperando uso real — claude (PR #1021-#1046; ver WORKLOG 2026-09-15).
  Eventos del turno con fotos antes/después, parada (MTTR) o ventana sin detener, pendientes que pasan
  solos al turno siguiente, técnicos del calendario, buscador de equipos; copiar al correo (Outlook
  clásico y nuevo) y PDF. Ronda 7: **Historial** del período (7/14/30 días) con tesis, KPIs y gráfico.
  Ronda 8: 10 arreglos de robustez de la entrega de turno.
  Falta: **prueba real de Orel en planta** (celular con fotos de cámara + pegado en su Outlook del PC).
  Producción sigue en 0 eventos / 0 fotos: hasta que se use un turno completo, el pulido va a ciegas.
...
- [x] **Solicitar a bodega end-to-end** — gap real: crear solicitud + notificaciones (Telegram/FCM) ya estaban cableadas, pero avanzar a "entregada" solo cambiaba la etiqueta de estado, sin tocar el inventario. Ahora "Entregar" registra un movimiento de salida real en `bodega` (misma `registrarMovimiento` que usa la vista manual) si el SAP ya está configurado ahí; si el descuento falla, el estado NO avanza; si el SAP nunca tuvo bodega, no se inventa un registro (evita choque con `isValidBodega()` por `ubicacionBodega` vacío). PR #175 mergeado y desplegado 2026-07-08. (claude — ver WORKLOG)
...
- [x] **Fase 5 — borrado de colecciones HECHO** (claude, 2026-06-20) — retiradas las features que leían `machines`/`plantAssets` (MachineProvider raíz sin consumidores, tab Categorías/CategoryManager, panel "Equipos SAP"), borrados 5 archivos (PR #81, deployado). Backup `backups/fase5-2026-06-20T17-25-56/` (9.303 docs). `11-delete-legacy.js --write` borró `insumos`(3552)+`machines`(36+5578 sub/huérfanas)+`plantAssets`(134)+`repuestosBaader200`(0)+`hierarchy/*/repuestos`(3). Verificado en vivo: las 5 en **0**; maestro 7657 / bodega 2170 / hierarchy 702 intactos. **Pendiente menor:** limpiar `EquipoPlacementTool` (inerte) + store `equipoToPlaceId` en `PlantaLeafletEditable.tsx`.
